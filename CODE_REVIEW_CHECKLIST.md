...
* `app/common/llm/base.py` — LLM 어댑터들의 공통 베이스 (완료)
...
* ~~`evals/eval_record.py`~~ — **삭제됨** (4단계 참고)
* `evals/eval_ragas.py` — Faithfulness/Answer Relevancy 직접 구현 로직 (리뷰 완료, 로드맵 참고)
...
* ~~`tests/test_record_chain_rules.py`~~, ~~`tests/test_record_chain_safety.py`~~ — **삭제됨**
