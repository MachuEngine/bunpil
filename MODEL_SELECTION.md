...
| 실측 수치 | 아래 비교표 + **2026-07-24 정기평가 재측정**(`evals/eval_exam.py`, 런타임과 동일 코드): 문항품질 κ=**0.468**✅(첫 목표 달성), ±1 일치율=0.700✅, 종합평균=4.06✅(첫 목표 달성 — 단 이 종합평균은 ITEM_GOLDEN 고정 골든셋 기준이라 **생성 품질이 아니라 Judge 신뢰도 측정값**이다. 실측 생성 품질은 `eval_ragas.py` 기준 3.14, EVAL.md §8.1). 구조유사도 difficulty_match 일치율=0.933, overall MAE=**0.644**(역대 최저) — EVAL.md §4 최신 행 |
