...
- **재시도 시 부분 진행 보존**: 기존엔 재시도마다 `init_session()`으로 전체 초기화(이전 시도 문항 폐기). `plan_node`가 요청당 1회만 초기화하고, `agent_node`는 `reset_judge()`만 호출해 문항을 유지, "나머지 N개만 작성" 프롬프트로 이어서 생성하도록 개선. `test_exam.py`로 정성 확인 완료(재시도 후에도 이전 시도 문항이 최종 결과에 유지됨). **old(전체 재시도) vs new(부분 보존) 정량 비교는 완료 못 함** — `scripts/test_retry_preservation.py`의 old 시뮬레이션이 재현 가능하게 첫 샘플에서 30분 이상 멈추는 현상이 반복됨(단발 수동 호출은 30초 만에 정상 응답 — 배치 실행에서만 재현, 원인 미규명). 20개 표본 시도 실패, 8개로 축소 재시도도 30분 초과로 중단. 다음 세션에서 원인 조사(예: 반복 invoke 시 커넥션 누적, 특정 프롬프트 조합에서의 서버 측 정체 등) 후 재시도 필요
