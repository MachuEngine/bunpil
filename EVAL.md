...
**다음 개선 방향**: rubric의 저점(0~2) 기준을 few-shot에 더 명시적으로 추가하거나,
overall_score를 이진이 아닌 순위 상관(Spearman) 위주로 보는 지표 전환을 검토.
...
| 구조 Judge 신뢰도 | diff κ 0.474 ✅ / overall 이진 κ 0.000 ❌ (14B, n=20, v3: 게이트+재시도 로직 후) | diff κ ≥ 0.4 달성 / overall κ ≥ 0.4 | overall MAE 1.000·±1 일치 0.800으로 연속형 지표는 크게 개선(v1 대비), 그러나 Judge 점수가 {3,4}에만 몰려 분산 0이라 이진 kappa가 수학적으로 0 — rubric 저점(0~2) 기준을 few-shot에 더 명시하거나 이진 대신 순위상관(Spearman) 지표로 전환 검토(5절 분석 참고) |
