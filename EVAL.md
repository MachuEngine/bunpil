...
**다음 개선 방향**: 표본을 늘려 노이즈를 줄이거나, overall_score를 이진 대신 순위 상관
(Spearman) 위주로 보는 지표 전환을 검토.
...
| 구조 Judge 신뢰도 | diff κ 0.687 ✅ / overall 이진 κ 0.178 ❌ (14B, n=20, v3+저점1+균형 few-shot) | diff κ ≥ 0.4 달성 / overall κ ≥ 0.4 | overall MAE 0.900·Pearson r 0.306으로 안정. 저점 few-shot을 2개 넣었더니 상관관계가 오히려 무너지는 부작용을 발견해(r 0.305→0.097) 저점 1개+균형 문구로 재조정, κ 0.000→0.178로 개선했으나 여전히 미달 — 표본 확대 또는 Spearman 위주 지표 전환 검토(5절 분석 참고) |
