...
def _build_system_prompt(
    ...
    return (
        ...
        # 원인이 명확해 보였다. 프롬프트에 유일성 지시가 아예 없다는 것도 확인했다(EVAL.md 22절).
        # 순차 배치로 전(3.375)/후(4.125) 비교했더니 +0.75가 나왔고 두 배치에서 반복돼
        # "재현됐다"고 판단했다.
        #
        # 그런데 **교차 실행 A/B(대조·처치를 번갈아 12회씩)로 재검증하니 효과가 사라졌다** —
        # 정답유일성 대조 3.083 vs 처치 3.000(p=0.58), 치명적 실패율은 양쪽 다 6/12로 동일.
        # 앞선 "재현"은 연속으로 돌린 두 배치가 같은 배치 효과를 공유한 것이었다(배치 간
        # 편차 3.0~4.125가 지시 효과보다 훨씬 컸다).
        #
        # 근거가 없는 292자를 매 호출 컨텍스트에 싣지 않기 위해 제거했다(이 프로젝트는
        # "입력이 길수록 언어 오염 확률이 오른다"를 실측한 적이 있다 — TROUBLESHOOTING 3번).
        # 정답유일성 자체는 여전히 열린 문제이며, 프롬프트가 아닌 수단이 필요하다.
        # 상세는 EVAL.md 23·24절.
        ...
        # ── few-shot 블록 (2026-08-07 실험) ─────────────────────────────
        # 텍스트 지시만으로는 정답유일성이 안 움직였다(교차 A/B p=0.58, EVAL.md 24절).
        # Judge 프롬프트에서는 few-shot 앵커가 실제로 지표를 움직인 전례가 있는데
        # (JUDGE_TPL 5점 앵커 2.50→2.83, STRUCTURE_JUDGE_TPL 3점 앵커) 생성기에는
        # 한 번도 예시를 준 적이 없다 — "말하기" 대신 "보여주기"를 시험한다.
        # 나쁜 예 2개는 실제로 우리 모델이 생성한 실패 사례다(지어낸 것이 아님).
        "[예시로 배우기]\n"
        "❌ 나쁜 예 1 — 정답이 2개라 잘못된 문항:\n"
        "  Q. 대한민국 정치 체제에 대한 설명으로 맞지 않은 것은?\n"
        "  ① 정당 제도가 있다(참) ② 대선은 10년 주기다(거짓) "
        "③ 의회는 단원제다(참) ④ 국회의장이 행정부를 이끈다(거짓)\n"
        "  → ②와 ④가 **둘 다 거짓**이라 답이 둘. 부정형에서는 나머지 3개가 모두 참이어야 한다.\n"
        "❌ 나쁜 예 2 — 정답이 유일하지만 너무 티남:\n"
        "  Q. 헌법이 보장하는 기본권으로 옳지 않은 것은?\n"
        "  ① 사생활의 비밀 ② 신체의 자유 ③ 종교의 자유 ④ 군사독재\n"
        "  → ④는 애초에 기본권 범주가 아니라 내용을 몰라도 맞힌다.\n"
        "✅ 좋은 예 — 답이 하나이면서 네 선지가 모두 같은 범주:\n"
        "  Q. 다음 중 시장 실패의 사례로 옳지 않은 것은?\n"
        "  ① 공공재의 무임승차(시장 실패 O) ② 독과점의 가격 결정(O) "
        "③ 환경 오염의 외부 효과(O) ④ 완전 경쟁 시장의 자원 배분(X)\n"
        "  → ④만 시장 실패가 아니라 답이 하나이고, 네 선지 모두 시장 개념이라 "
        "내용을 알아야 판별된다.\n\n"
        f"{no_text_rule}"
    )
