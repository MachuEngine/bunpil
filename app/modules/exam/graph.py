...
def _build_system_prompt(
    ...
    return (
        ...
        # 2026-08-07 추가: 정답유일성 미달(3.375/4.0)의 원인은 "지시가 약해서"가 아니라
        # **아예 지시가 없어서**였다 — 실측 8건 중 3건이 조건을 만족하는 선지가 2개인
        # 파손 문항이었고(EVAL.md 22절), 특히 부정형에서 자주 깨진다. 코드로는 판정할 수
        # 없는 의미 검증이라(선지 내용의 참/거짓) 생성 단계에서 자가 점검시킨다.
        "**선지를 다 쓴 뒤, 질문의 조건을 만족하는 선지가 정확히 하나인지 반드시 확인하세요.**\n"
        "- '옳은 것은?'처럼 긍정형이면 → 참인 선지가 **1개**, 나머지 3개는 모두 거짓이어야 합니다.\n"
        "- '옳지 않은 것은?'처럼 부정형이면 → 거짓인 선지가 **1개**, 나머지 3개는 모두 참이어야 합니다.\n"
        "조건을 만족하는 선지가 2개 이상이면 정답이 유일하지 않은 잘못된 문항입니다 — "
        "선지를 고쳐 쓴 뒤 저장하세요. 특히 부정형은 참인 진술 3개를 정확히 만들어야 하므로 "
        "각 선지가 실제로 사실인지 하나씩 따져보세요.\n\n"
        # 2026-08-07 시행착오 기록: 위 유일성 지시의 부작용(유일성을 확보하려고 정답 선지를
        # 노골적으로 만듦 — "기본권으로 옳지 않은 것은?" → ④ 군사독재)을 막으려고 균형 문구를
        # 추가해봤으나 **효과 없어 되돌렸다**. 3기준 평균이 전혀 안 움직였고(4.125/3.5/4.375로
        # 동일), 결정적으로 **문구가 명시적으로 지목한 '군사독재' 사례가 그대로 재현**됐다.
        # 텍스트 지시로는 이 실패 모드를 못 막는다는 뜻 — 상세는 EVAL.md 23절.
