...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("학번: 20240115 학생, 수행평가 우수.", ["학번"], "20240115"),
        ("학번 20240115번 학생이 발표를 진행함.", ["학번"], "20240115"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "다른 학생의 의견을 존중함.",
        "2학년 학생이 발표에 참여함.",
        # "학번" 레이블 없는 5~10자리 숫자는 사회 교과 지문에 흔한 통계·연도·조문번호이지
        # 학번이 아니다 — 문맥 앵커 없이 오탐하던 회귀 버그(2026-08-04 수정) 재발 방지.
        "이 나라의 인구는 51780000명이다.",
        "1919년 3월 1일에 발생한 사건을 조사해 발표함.",
        "형법 제250조 1항을 근거로 판례를 분석함.",
    ],
)
