...
| 문항 품질 — 정답유일성 3.875 / 오답매력도 3.82 (목표 4.0) | 🔄 정답유일성 진행 중 · ⏸️ 오답매력도 종결 — 텍스트 지시는 무효(p=0.58, 되돌림)였으나 **few-shot(예시로 보여주기)이 정답유일성을 3.091→3.875까지 개선**(치명적 실패 36%→21%, 미확정). 오답매력도는 네 가지 시도(텍스트 지시·few-shot·어휘 게이트·의미 게이트)가 모두 무위로 끝나 **개선 수단 없음으로 종결** — 82%가 4점에 뭉쳐 저품질 비율(≤3점, 현재 15%)로 추적 전환. 둘 다 런타임 게이트가 아닌 오프라인 지표([EVAL.md](./EVAL.md) 24~27절) |
...
> 지표 전체 목록·골든셋 현황·결과 이력은 [EVAL.md](./EVAL.md), 지금 상태 한 장 요약은 [EVAL_SUMMARY.md](./EVAL_SUMMARY.md)에서 계속 갱신합니다. 아래는 **지표별 최신 실측치**입니다(생성 Qwen2.5-14B / Judge gpt-5.6-luna — 현재 배포 구성과 동일). Judge 신뢰도·문항 품질은 2026-07-24(judge 분리 후 첫 정식 측정), 검색 Recall@5/MRR은 2026-08-03(`n_candidates` 조정 후, 더 최신) 기준이라 날짜가 섞여 있습니다 — 지표별 근거는 [EVAL.md](./EVAL.md) 4절 결과 이력 참고.

| 지표 | 값 | 의미 |
|---|---|---|
| Judge 신뢰도 (Cohen's κ) | **0.468** | LLM Judge 채점이 사람 라벨과 일치한다는 근거. 이게 없으면 아래 종합평균도 신뢰할 수 없습니다 |
| 문항 품질 종합평균 | **4.06 / 5.0** | 실제 산출물 품질 (목표 4.0 첫 달성) |
| 검색 Recall@5 | **1.000** (MRR 0.854) | RAG 파이프라인이 실제로 관련 문서를 찾아온다는 근거 |

<details>
<summary><b>상세 지표 — 검색·Judge 신뢰도·구조 유사도 (펼치기)</b></summary>
