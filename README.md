...
- 생성물 기준 수치는 그동안 `experiments/`의 **일회성 A/B로만** 산발적으로 재 왔습니다. 오답매력도는 3.82였고, 정답유일성은 텍스트 지시가 통하지 않았습니다(p=0.58, 관찰된 차이가 우연일 가능성이 58%라 효과를 인정할 수 없다는 뜻). few-shot으로는 3.091에서 **3.875**까지 올랐지만 아직 미확정입니다
- ⚠️ 위 값들은 표본도 조건도 제각각이라, 아래 **정기 관측치(오답매력도 3.29, 정답유일성 2.67)** 쪽이 현재 기준입니다
