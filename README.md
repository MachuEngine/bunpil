...
| 검색 MRR | 22 | 참고값 | **0.814** (하이브리드 검색 도입 후, 이전 dense-only 0.789) |
...
- **하이브리드 검색(2026-08-03)**: dense 단독이던 검색에 BM25 어휘 검색을 더해 RRF로 융합. 새 모델·재인덱싱 없이 BGE-M3 토크나이저와 기존 청크 텍스트만 사용 — 전체 MRR 0.789→0.814, standards MRR 0.892→0.938. **다만 원래 목표였던 regulations MRR(0.667)은 개선되지 않았고**, 대신 dense가 후보에조차 못 올리던 청크를 BM25가 찾아냈는데 리랭커가 top-5로 못 올린다는 것이 드러나 **병목이 검색기 → 리랭커로 이동**(EVAL.md 12절)
...
> `similarity_judge` 도구 호출이 잡혀, 신·구 아키텍처 트레이스가 한 통계에 섞여 있었음을
> 확인 → `--since` 옵션 추가. 이 때문에 **첫 실행 수치는 현재 아키텍처의 성능이 아니며**,
> 신 아키텍처만 놓은 재측정은 LangSmith 무료 플랜 한도 소진으로 **미완**입니다(EVAL.md 11절).
