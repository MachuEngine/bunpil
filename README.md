...
세트 전체는 LangGraph 그래프가 관리합니다. `agent`가 `submit_for_review`로 제출하면 `judge` 노드가 **생성 모델과 완전히 분리된 Judge 백엔드**(`get_judge_backend()`)로 구조 유사도를 채점합니다. 그다음 `validate` 노드가 문항 개수 일치와 Judge 결과 threshold를 코드로 판정합니다.
...
텍스트를 옮겨 적는 대신 화면 캡처를 붙여넣을 수 있습니다. 입력창에 이미지를 `Ctrl+V`로 붙여넣거나 "이미지 첨부" 버튼으로 파일을 고르면 VLM(기본 OpenAI, `VLM_BACKEND`)이 텍스트를 추출합니다. 발문과 `<보기>`, 선지는 원문 그대로 옮기고, 표·그래프·지도처럼 글자가 아닌 자료는 `[자료: ...]`로 풀어 씁니다.
...
> 정확도: text_only CER 0.0071 / figure CER 0.1066 (2026-08-20 최초 측정, `evals/eval_vlm.py`, 40건 골든셋 — 상세는 [MODEL_SELECTION.md](./MODEL_SELECTION.md) §7)
...
같은 샘플 15개와 고정 Judge로 비교했습니다. 재시도가 없는 조건(`budget=1`)에서는
...
손봐도 사람 라벨과의 신뢰도(Cohen's κ)가 목표치 0.4를 넘지 못했습니다. 생성물을 고정한 채
Judge만 qwen2.5:7b에서 gpt-5.6-luna로 바꿔 비교하자 문항 품질 κ가 0.328에서 0.595로,
구조 유사도 overall MAE가 1.689에서 0.600으로 개선돼 채택했습니다. 생성 모델은 비용과
...
> 회차별 raw 로그는 [EVAL.md](./EVAL.md) — 시간순 실험 로그라 뒤 절이 앞 절을 뒤집는 곳이 있어, 어디가 그런지는 그 문서 0절에 색인해 뒀습니다.

아래 4개가 헤드라인입니다. **측정 조건이 행마다 다르니 세 번째 열을 같이 읽어 주세요.**

검색과 Judge 신뢰도는 배포 구성(생성 Qwen2.5-14B, Judge gpt-5.6-luna)에서 잰 값이지만, 실측 생성 품질만 더 작은 모델과 더 낮은 budget에서 측정했습니다. 잰 날짜도 섞여 있습니다(Judge 신뢰도 2026-07-24, 검색 2026-08-03, 실측 생성 품질 2026-09-07).
...
- 생성물 기준 수치는 그동안 `experiments/`의 **일회성 A/B로만** 산발적으로 재 왔습니다 — 오답매력도 3.82, 정답유일성은 텍스트 지시가 무효(p=0.58)였으나 few-shot으로 3.091→**3.875**(미확정). ⚠️ 표본도 조건도 제각각이라, 아래 **정기 관측치(오답매력도 3.29 · 정답유일성 2.67)** 쪽이 현재 기준입니다
