...
| 정답유일성(~3.0/4.0) | ⬜ 미달 — 추적하던 "오답매력도 2.846"이 구버전 스택 값이라 무효였고, 재측정하니 진짜 병목은 정답유일성이었다(**정답이 2개인 문항이 50%**). 프롬프트 지시로 +0.75를 얻은 듯했으나 **교차 A/B로 재검증하니 배치 효과였고 실제 효과는 없어**(p=0.58) 되돌렸다 — 텍스트 지시로는 해결되지 않는 문제([EVAL.md](./EVAL.md) 22·24절) |
