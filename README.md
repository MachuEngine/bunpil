...
| 문항 품질 (오프라인 지표, 런타임 게이트 아님) | 🔄 정답유일성 **3.875** · ⏸️ 오답매력도 **3.82** 종결 — 네 번 시도해 한 번 통했습니다([EVAL_SUMMARY 4.7절](./EVAL_SUMMARY.md#47-문항-품질-개선--네-번-시도해-한-번-통했다-2026-08-08)) |
...
아래 4개가 헤드라인입니다. **측정 조건이 행마다 다르니 세 번째 열을 같이 읽어 주세요** — 검색·Judge 신뢰도는 배포 구성(생성 Qwen2.5-14B / Judge gpt-5.6-luna)이지만, 실측 생성 품질만 더 작은 모델·더 낮은 budget에서 잰 값입니다. 날짜도 섞여 있습니다(Judge 신뢰도 2026-07-24 · 검색 2026-08-03 · 실측 생성 품질 2026-09-07).
...
- 문항 품질·Judge 신뢰도가 이번에 나란히 첫 목표 달성 — Judge를 gpt-5.6-luna로 교체한 효과가 큼. 세부 항목 중 오답매력도가 3.40으로 낮게 나왔으나, 이는 **고정 골든셋(ITEM_GOLDEN) 채점값이라 우리 생성 품질이 아니다**(EVAL.md 9절). 실제 생성한 문항 기준으로 다시 재보면 오답매력도는 **3.82**로 사실상 해결된 값이었고(ITEM_GOLDEN 기준 3.50과는 모집단이 다름), 진짜 병목은 **정답유일성**이었다 — 텍스트 지시는 무효(p=0.58)였으나 few-shot으로 3.091→**3.875**까지 개선했다(미확정). 오답매력도 자체는 이후 네 가지 개선 시도가 모두 무위로 끝나 **개선 수단 없음으로 종결**했다(EVAL.md 22·24~27절, 현재 상태는 [EVAL_SUMMARY.md](./EVAL_SUMMARY.md) §5). ⚠️ 여기 적힌 생성물 기준 수치(오답매력도 3.82·정답유일성 3.875)는 `experiments/`의 **일회성 A/B에서 나온 값**이라, 아래 정기 관측치(각 3.29·2.67)와 표본도 조건도 다릅니다
...
**개수·형식 게이트는 전부 통과하고 Judge 점수에서만 막힙니다.** 문항 품질 미달(정답유일성은 few-shot으로 3.875까지 개선 중, 오답매력도는 개선 수단 없음으로 종결, 2026-08-08 기준)이라는
