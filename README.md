...
| 정답유일성(4.125/4.0) | ✅ 달성 — **2026-08-07 타깃 전환 후 개입**. 추적하던 "오답매력도 2.846"은 7B 생성+qwen Judge 시절 값이라 무효였고, 재측정하니 진짜 병목은 정답유일성(3.375)이었다(8건 중 3건이 **실제로 정답이 2개**). 프롬프트에 유일성 자가 점검 지시를 추가해 **4.125**로 개선(독립 2배치 재현, 치명적 실패 37.5%→12.5%). 오답매력도는 3.5로 소폭 하락 — 트레이드오프 가능성([EVAL.md](./EVAL.md) 22·23절) |
