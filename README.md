...
> 상세 방법론·전체 결과·raw 데이터는 [EVAL.md](./EVAL.md)의 "7. 모델 비교 실험" 절 참고.

Qwen2.5-7B(현재 프로덕션 기본값)를 정식 채택 모델로 확정하지 않고, 후보들을 동등 조건
(동일 passage_text 15개, 자기채점 편향을 피하기 위해 고정된 하나의 Judge로 통일, 재시도
없는 조건)으로 비교했다.

| 모델 | 실패율(0문항) | 평균 생성시간 | 개수 충족률 |
|---|---|---|---|
| Qwen2.5-7B | 40% | 50.3s | 0.15 |
| Qwen2.5-14B | 0% | 158.1s | 0.76 |
| Llama3.1-8B | 60% | 59.4s | 0.11 |
| GPT-4o-mini | **0%** | **18.1s** | **0.97** |
| Qwen3.5-9B | 26.7% | 171.1s(최저 속도) | 0.55 |

GPT-4o-mini가 실패율·속도·개수 충족률 등 안정성 지표를 전부 석권했으나, **최종 채택은 아직
검토 중이며 프로덕션은 여전히 Qwen2.5-7B(로컬)를 사용 중**이다. 비용(소액이지만 0은 아님)과
외부 API 의존성 트레이드오프를 고려해 결정할 예정. Qwen3.5-9B는 문항 품질(정답유일성)은
5개 중 가장 높았지만 속도가 가장 느려 로컬 대안으로도 뚜렷한 우위가 없었다.
...
| `LANGCHAIN_PROJECT` | LangSmith 프로젝트 베이스명 — `LLM_BACKEND`에 따라 실제로는 `-dev`(local/openai) 또는 `-prod`(runpod) 접미사가 자동으로 붙음(`app/common/llm/tracing.py`). 로컬 개발 노이즈가 프로덕션 통계를 오염시키지 않도록 분리 | `bunpil` → `bunpil-dev` / `bunpil-prod` |
