...
- 문항 품질·Judge 신뢰도가 이번에 나란히 첫 목표 달성 — Judge를 gpt-5.6-luna로 교체한 효과가 큼. 세부 항목 중 오답매력도가 3.40으로 낮게 나왔으나, 이는 **고정 골든셋(ITEM_GOLDEN) 채점값이라 우리 생성 품질이 아니다**(EVAL.md 9절; 이후 재측정한 ITEM_GOLDEN 값은 3.82). 실제 생성한 문항으로 재보면 진짜 병목은 **정답유일성**이었다 — 텍스트 지시는 무효(p=0.58)였으나 few-shot으로 3.091→**3.875**까지 개선했다(미확정). 오답매력도는 네 가지 개선 시도가 모두 무위로 끝나 **개선 수단 없음으로 종결**했다(EVAL.md 22·24~27절)
