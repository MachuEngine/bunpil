...
- **목적**: 고등학교 사회 교사의 문제 출제를 돕는 대화형 웹 서비스
- **사용 맥락**: 포트폴리오(Agent·RAG·평가·배포 실습) + 지인 교사 1인 실사용
- **모듈**: ② 출제 도우미(Agent) — ③ 생기부 윤문(Chain)은 2026-08-03 제거
...
> **2026-09-24 추가 — 형식 인식·해설·챗봇 수정·다운로드**
>
> - **형식 인식**: 문항에 `stimulus`(`<보기>`·자료 제시문) 필드를 추가하고, 예시 원문을 보고 코드가
>   형식을 정한다 — `⑤`가 있으면 5지선다, `<보기>`·`[자료`가 있으면 객관식은 `stimulus` 필수,
>   선지가 `① ㄱ, ㄴ` 꼴이면 합답형(선지는 `<보기>` 기호 조합만, 기호는 `<보기>`에 있어야 함).
>   프롬프트 지시만으로는 14B가 형식을 따르지 않았고 Judge도 감점하지 않아 결정론적 게이트로 막았다
>   (`tools.py` `_format_errors`). Judge에는 `stimulus`가 있을 때만 넘겨, 제시문 없는 문항은 입력이 기존과 같다.
> - **해설**(`POST /exam/explain`, `explain.py`): 버튼을 누를 때 생성. 이미 정한 정답을 주고 근거만 쓰게 한다.
> - **챗봇 수정**(`POST /exam/revise`, `revise.py`): 바뀐 문항만 돌려받아 생성 경로와 같은 게이트
>   (형식·한국어·예시 복사·세트 내 중복)로 검사하고, 실패하면 한 번 다시 쓰게 한 뒤 그래도 실패하면 원본 유지.
>   예시 형식 규칙은 적용하지 않는다(교사가 다른 형식을 일부러 요청할 수 있음) — 요청에 "합답형"·`<보기>`가
>   있을 때만 그 규칙을 적용. Judge 재채점 없음.
> - 해설·수정은 **그래프 밖 단발 호출**이라 노드 순서는 그대로다. 서버는 여전히 stateless — 브라우저가
>   생성 문항·대화를 들고 있다가 매 요청에 다시 보내고, 서버는 이를 사용자 입력으로 보고 모델 호출 전에 마스킹한다.
> - **다운로드**(학생용·교사용 PDF/TXT/PNG): 전부 브라우저에서 생성(`frontend/lib/export.ts`), 서버 관여 없음.
...
관찰 메모를 받아 `mask_pii → polish → validate`로 처리하던 검증 Chain이었다. 제거 사유:
검증 규칙(종교·외모·추측 등 키워드 6종)의 근거를 추적한 결과 인덱싱된 교육부 기재요령
원문에 해당 조항이 **한 건도 없었고**(EVAL.md 14절), 규칙의 실제 출처는 합성 골든셋의
라벨이었다. 게다가 키워드가 서술 대상을 구분하지 못해 사회 교과 문장("정치적 다원주의
개념을 조사해 발표함")을 막고 정작 규정이 금지하는 문장("아버지가 대기업 임원이라…")은
통과시켰다. 하드룰 1 때문에 실제 데이터로 검증할 수도 없어 범위에서 제외했다.

PII 마스킹(`app/common/privacy.py`)은 출제 경로가 계속 쓰므로 유지된다 — `app/main.py`
`_build_spec()`이 `passage_text`를 모델에 넘기기 전에 호출한다.
...
- **GitHub Actions CI** (2026-07-14 결정·구현 완료): 코드 회귀 확인용 **경량 CI만 도입**, LLM
  ...
  - **eval 자동화를 뺀 이유**: `JUDGE_BACKEND`로 Ollama/OpenAI Judge를 선택할 수 있지만,
    ...
    `evals/eval_exam.py`/`evals/eval_ragas.py`는 변경 없이 로컬 실행 스크립트로 유지
