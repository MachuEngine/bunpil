...
관찰 메모 입력 → 개인정보 마스킹 → 윤문 생성 → 규정·사실보존 검증 → 안전 출력 + 책임 고지
                                                       ├ 차단 사유 → 재시도, 해소 실패 시 출력 보류
                                                       └ 경고 → 출력하되 교사에게 확인 요청
```

> **차단과 경고 구분(2026-08-03)**: 출력을 보류하는 것은 CLAUDE.md 하드룰에 직접 걸리는
> 항목뿐이다 — PII 생성·잔존(하드룰 2·4), 메모에 없는 사실 추가(하드룰 5), 검증 시스템
> 실패(fail-closed). 키워드 규칙(종교·정치·외모·가정환경 등)과 LLM 규정 판정은 **경고로
> 강등**했다. 키워드는 "학생 본인의 속성"과 "학생이 탐구한 주제"를 구분하지 못해 사회 교과
> 문장을 오탐하고("사회 수업에서 정치적 다원주의 개념을 조사해 발표함" 등), 정작 규정이
> 금지하는 "아버지가 대기업 임원이라…"는 놓쳤다. 최종 기재 책임이 교사에게 있으므로
> 코드가 결과를 숨기는 대신 판단 재료를 넘긴다. 상세는 EVAL.md 14절.
