...
- 🔄 진행 중: 코드 리뷰(아래 "참고 — 코드 리뷰 대상 파일" 표는 예전 스냅샷 — 신뢰 금지, 진행 상황은 로컬 `CODE_REVIEW_CHECKLIST.md`(저장소 미포함)로 추적)
...
| 2-1 | **문항 품질 — 실측 생성물 종합 3.14**(정답유일성 2.67 / 오답매력도 3.29, 2026-09-07 정기 관측) | 🔄 정답유일성 진행 중 · ⏸️ 오답매력도 종결 | **정답유일성**: 텍스트 지시는 무효(p=0.58, 되돌림)였으나 few-shot이 개선(치명적 실패 36%→21%, EVAL.md 25절). **오답매력도**: 네 접근(텍스트 지시·few-shot·어휘 게이트·의미 게이트) 모두 근거 부족으로 끝나 **현재 수단으로는 개선 불가**로 종결(27절). 채점기는 정상이나 82%가 4점에 뭉쳐 평균 해상도가 낮아 저품질 비율(≤3점)로 추적(26절). **둘 다 런타임 게이트가 아닌 오프라인 지표** — validate_node는 count_match + 구조 유사도 3지표만 본다. **근본 해결에는 기준별 사람 라벨링이 선행 필요**. ⚠️ 여기 적힌 3.875/3.82는 `experiments/` 일회성 A/B 값이고, `eval_ragas.py` 정기 관측치는 2.67/3.29다(EVAL.md 8.1절) | 기존 열린 이슈(아래 1번 항목). `agent_node` few-shot이 텍스트 지시문뿐이라 실효성이 약할 수 있음 — 진짜 멀티턴 tool-call 예시로 강화하거나 `validate_item_format`에 최소 기준 게이트 추가 검토 |
| 3 | **코드 리뷰 전수 확인** | 🔄 진행 중 | 하단 "참고 — 코드 리뷰 대상 파일" 표는 예전 스냅샷이라 신뢰 금지. 체크리스트는 로컬 `CODE_REVIEW_CHECKLIST.md`(저장소 미포함)로 이전(2026-08-04, 경로 오류 6곳 정정 + 신규 파일 5개 반영). 2026-08-03에 `tools.py`·`retriever.py`·`chain.py`(삭제됨)를 실제로 훑으며 도구-코퍼스 불일치를 발견한 것이 부분 진행분 |
...
| 7 | **포트폴리오 정리** | 🔄 진행 중 | 아키텍처 다이어그램(README mermaid·SVG 갱신 완료), LangSmith 트레이스 스크린샷(1번 해소 후), 기술 블로그 초안(로컬 `blog_draft.md`, 저장소 미포함) |
...
1. **문항 품질 — 정답유일성 3.875 / 오답매력도 3.82 (목표 4.0)** — 2026-08-07 타깃 전환
   (구 "오답매력도" 항목), 2026-08-08 각각 별도 결론
   ...
   >
   > **2026-08-07 프롬프트 지시 개입** — 자가 점검 지시로 3.375→4.125를 얻은 줄 알았으나,
   > 대조·처치를 교차 실행하는 A/B(`experiments/ab_uniqueness_instruction.py`)로 재검증하니
   > **배치 효과였고 실제 효과는 없었다**(대조 3.083 vs 처치 3.000, p=0.58) — 지시는
   > 되돌렸다(EVAL.md 24절).
   >
   > **2026-08-07~08 few-shot 개입** — "말하기" 대신 "보여주기"로 메커니즘을 바꿔 생성
   > 프롬프트에 나쁜 예 2개 + 좋은 예 1개를 추가하자, 같은 교차 A/B에서 **정답유일성만
   > 개선됐다**(3.091→**3.875**, 치명적 실패 36%→21%, p=0.0486이나 sequential testing
   > 보정 전이라 확정은 아님). 오답매력도는 이 개입에도 무반응(+0.053, p=0.83, EVAL.md 25절).
   >
   > **2026-08-08 오답매력도 — 개선 수단 없음으로 종결** — 채점기 자체를 검증해보니 정상
   > 작동하지만(오답 품질 3단계 구분 시 4/2/1로 정확히 갈림), 실측 62건 분포의 **82%가
   > 4점에 뭉쳐 있어** 평균의 해상도가 낮았다. 텍스트 지시·few-shot·어휘 게이트(선지
   > 재사용)·의미 게이트(임베딩 유사도) **네 가지를 다 시도했으나 전부 근거 부족**으로
   > 끝나(EVAL.md 26·27절) 지금 수단으로는 개선 불가로 결론짓고, 대신 **저품질 비율
   > (≤3점)을 추적 지표로 추가**했다(현재 15%). **둘 다 런타임 게이트가 아니다** —
   > `validate_node`는 `count_match` + 구조 유사도 3지표만 보고, `judge_one`은
   > `evals/`·`experiments/`에서만 호출돼 교사에게 나가는 결과에는 영향이 없다.
...
6. **GitHub Actions CI** (2026-07-14 완료 — 경량 CI만 도입, eval 자동화는 도입 안 하기로 결정. 상세는 DESIGN.md 9절 참고)
   ...
   - **eval 자동화는 도입 안 함**: 설계 검토 중 `get_judge_backend()`(factory.py)가 `LLM_BACKEND`
     ...
     대비 이득 적어 제외
7. **문서화 및 포트폴리오 정리** — README/DESIGN.md/`MODEL_SELECTION.md` 최신 모델 결정 반영 완료(2026-07-22). 남은 것: 아키텍처 다이어그램(README에 mermaid로 이미 있음, 스크린샷 별도 불필요할 수 있음) 확인, LangSmith 트레이스 스크린샷, 기술 블로그 초안(로컬 `blog_draft.md`, 저장소 미포함 — 진행 상태 확인 필요)
