...
| 2-1 | **문항 품질 — 정답유일성 3.875 / 오답매력도 3.82** | 🔄 정답유일성 진행 중 · ⏸️ 오답매력도 종결 | **정답유일성**: 텍스트 지시는 무효(p=0.58, 되돌림)였으나 few-shot이 개선(치명적 실패 36%→21%, EVAL.md 25절). **오답매력도**: 네 접근(텍스트 지시·few-shot·어휘 게이트·의미 게이트) 모두 근거 부족으로 끝나 **현재 수단으로는 개선 불가**로 종결(27절). 채점기는 정상이나 82%가 4점에 뭉쳐 평균 해상도가 낮아 저품질 비율(≤3점)로 추적(26절). **둘 다 런타임 게이트가 아닌 오프라인 지표** — validate_node는 count_match + 구조 유사도 3지표만 본다. **근본 해결에는 기준별 사람 라벨링이 선행 필요** | 기존 열린 이슈(아래 1번 항목). `agent_node` few-shot이 텍스트 지시문뿐이라 실효성이 약할 수 있음 — 진짜 멀티턴 tool-call 예시로 강화하거나 `validate_item_format`에 최소 기준 게이트 추가 검토 |
