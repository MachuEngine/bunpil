...
| 2 | **`record_score` self-judge + Judge payload 불일치** | ✅ ① 조치 완료(2026-08-05) · ⬜ ② 미착수 | ① **완료** — 자기채점을 validate 게이트에서 제외하고 참고값으로 강등, 미채점(`unscored`)과 저품질(`rejected`) 분리, UI를 "AI 자체평가"로 라벨링(EVAL.md 16절). 단 **자기채점의 정확도 자체는 여전히 미측정** — `judge_one`(κ=0.468)과 비교하면 사람 라벨 없이 정량화 가능. ② **미착수** — 런타임은 자기채점 점수가 포함된 9개 필드를, 오프라인 eval은 5개 필드를 Judge에 넘김(검증-배포 입력 불일치). `judge_node`에서 필드를 걸러내면 되는 소규모 수정 |
