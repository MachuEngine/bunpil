...
| 2-1 | **문항 품질 — 정답유일성 3.875 / 오답매력도 3.82 (목표 4.0)** | 🔄 진행 중 | 텍스트 지시는 무효(p=0.58, 되돌림)였으나 **few-shot은 정답유일성을 개선**(치명적 실패 36%→21%, EVAL.md 25절). 오답매력도는 두 개입 모두 무반응이라 채점기를 검증했더니 **82%가 4점에 뭉쳐 평균의 해상도가 낮았다** — 저품질 비율(≤3점) 지표를 추가해 추적 방식을 바꿨다(EVAL.md 26절). **남은 한계**: 기준별 사람 라벨이 없어 오답매력도 채점 신뢰도 자체는 미검증 | 기존 열린 이슈(아래 1번 항목). `agent_node` few-shot이 텍스트 지시문뿐이라 실효성이 약할 수 있음 — 진짜 멀티턴 tool-call 예시로 강화하거나 `validate_item_format`에 최소 기준 게이트 추가 검토 |
