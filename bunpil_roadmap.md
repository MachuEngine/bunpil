...
2. **생기부 규정 위반 Recall 미달 + 알려진 RAG 우회 리스크 (0.927 / 목표 0.95)**
   ...
   - **다만 이 열린 이슈 자체는 아직 닫지 않음**: 개선된 것은 `retrieval_golden_final.json`
     22건 기준이고, 이 이슈의 실제 우려는 **"골든셋에 없는 새 표현으로 가정환경/종교를
     언급하는 문장"** 이다. 그 유형의 골든 항목이 아직 없어 하이브리드가 실제로 그걸
     잡는지는 미측정 — 골든 항목 추가(사람 라벨링 필요) 후 위반 Recall 재측정이 남음
