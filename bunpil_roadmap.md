...
4. **출제 에이전트 안정성 개선 (컨텍스트/tool-calling, 2026-07-10 야간 자율 세션)** — TROUBLESHOOTING.md 참고. 세부 내용:
   ...
   - **3단계(재시도 구조 개선)**: 재시도마다 init_session()으로 전체 초기화하던 것을 plan_node(요청당 1회)로 옮기고, agent_node는 reset_judge()만 호출해 기존 문항을 유지. 부족분만 "나머지 N개만 작성" 프롬프트로 이어서 생성(_build_system_prompt). **구현 완료, test_exam.py로 정성 확인**(재시도해도 이전 문항 유지됨). **정량 비교(old vs new) 미완료**: test_retry_preservation.py가 old 시뮬레이션 첫 샘플에서 30분+ 멈추는 현상이 재현됨(20개→8개로 축소해도 동일) — 단발 수동 호출은 정상(30초)이라 배치 실행 특유의 문제로 추정, 원인 미규명. 다음 세션 과제로 남김(EVAL.md 5절 참고)
