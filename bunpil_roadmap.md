...
| 2-1 | **정답유일성 ~3.0 / 목표 4.0** (구 오답매력도 항목에서 2026-08-07 전환) | ⬜ 미해결 — 프롬프트 개입 시도했으나 교차 A/B에서 효과 없음이 확인돼 되돌림(EVAL.md 24절) | 기존 열린 이슈(아래 1번 항목). `agent_node` few-shot이 텍스트 지시문뿐이라 실효성이 약할 수 있음 — 진짜 멀티턴 tool-call 예시로 강화하거나 `validate_item_format`에 최소 기준 게이트 추가 검토 |
