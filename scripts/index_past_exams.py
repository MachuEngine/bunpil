"""data/past_exams/ 디렉토리의 PDF를 past_exams 영구 컬렉션에 적재.
이미 적재된 파일(source 기준)은 스킵 — idempotent.
실행: python scripts/index_past_exams.py
"""
import logging
import os
import sys
...
def main() -> None:
    ...
    if not pdf_files:
        logger.warning("data/past_exams/ 에 PDF 파일이 없습니다.")
        return
    ...
    logger.info("완료 — 이번 실행 적재: %d 파일 / past_exams 총 문서 수: %d", indexed, total)
